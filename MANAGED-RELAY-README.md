...
正常基准固定为用户于 2026-09-20 确认的本次 INPUT 截图，作为项目资源分发；重新测试、更换配置目录、重启服务均不会更新基准。请求前校验图片 SHA-256，缺失或变化时报评审失败。设置中可点击「查看固定 INPUT 正常基准」。评审提示词采用已确认文本，保存在 `managed-relay-runtime/intelligence-review-prompt.txt`；每次评审记录基准图和提示词的指纹，方便核对使用的版本。 同一份规则同时放入 Responses 顶层 `instructions` 和用户消息首个文本块，兼容会替换或未遵循顶层指令的中转。材料块仍放在规则之后，不会将模型生成的代码提升为评审指令。
