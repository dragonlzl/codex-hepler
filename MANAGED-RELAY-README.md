...
- **已确认不足不会因查询失败消失**：记录最近确认不足或套餐失效的结果，查询超时、登录过期、数据过期、返回缺失以及额度重置时间到来，都不能解除排除。只有对应扣费来源、对应套餐的一次更新的成功查询确认达标才恢复；模型调用成功或另一种额度查询成功不能代替。无备用入口时仍保留最后路由，但不会把该入口标为合格；备用恢复后立即换站。上次达标或从未查询成功的入口，查询失败仍按“待确认”处理。
- **额度记录的生命周期**：负面额度记录由自动切换服务单独保存在当前配置目录的 `relay-ui-runtime/auto-switch-funding.json`，不包含 API Key 或登录凭据。自动切换开关、重新登录、改名、修改优先级和服务重启不会清除记录；更换 Key/地址、账号绑定、扣费来源、关联套餐或移出候选池后不沿用旧入口记录。修改 M/N 按新阈值重新判断旧金额：仍低于则继续排除，不再低于则不再作为不足证据，但查询失败仍不能确认达标；零额度始终不合格。保存失败时当前进程仍保留排除并提示，后续检测重试保存。
