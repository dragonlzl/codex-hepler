...
- 刷新失败会区分单项上游查询超时、连接重置、HTTP 错误或数据格式错误；浏览器取不到整次刷新结果时，单独提示管理服务超时或 HTTP 状态。状态、余额及订阅保留各自上次成功数据和时间；查询失败的时间、资源类别和安全错误码记录到既有诊断日志，不保存凭据或原始响应。刷新与智力测试使用独立取消信号和连接对象，刷新失败不会取消模型任务；两者仍可能同时受到所选代理或网络故障影响。
