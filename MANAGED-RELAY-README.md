...
- **先过滤资格，再排序**：未授权、授权过期、余额查询失败或过期的入口不参与选择，即使有优先级或有效订阅。使用 API Key 直接查询额度的入口，以 Key 查询成功作为授权依据，无需额外网页登录。订阅入口要求指定套餐有效、额度可确认且不低于 M；余额入口要求余额不低于 N。等于阈值仍合格，实际额度为零时始终不合格。余额低于 N 不影响一个仍满足 M 且余额查询正常的订阅入口。
...
- **恢复窗口**：切入候选使用当前状态及最近 3 分钟内全部检测；出现红色、未知或缺失格则继续等待，有黄色则按黄色处理。单格周期超过 3 分钟的状态源使用最近一格，仍遵守该来源的过期时间。刷新同一格不构成新恢复证据。固定但没有真实时间戳的状态、混合模型参考统计不能用于自动选路。当前渠道已恢复时取消故障计时，不因它自己的旧红格而强行换到较低优先级。
- **故障等待**：正在使用的池内入口明确不可用时，保留当前路由，每 15 秒重新检测；连续不可用满 3 分钟才换站。期间恢复则取消计时，下一次故障重新计时。认证或额度资格失效不适用这段宽限期。检测数据未知不冒充红色故障；有合格替代项时可重新选择，无候选时保留最后路由并继续检查整个池子。
- **实际调用反馈**：上游服务错误、限流或连接失败会阻止旧绿格立即把故障入口重新选入，当前入口仍遵守 3 分钟等待；新的成功请求或故障后至少 3 分钟且有更新的合格样本才能解除。调用 Key 的 401/403 立即排除，需修正 Key 或重新授权。客户端取消、普通参数错误及本地出站代理配置错误不触发渠道故障。转发层的 HTTP 成功不是对所有模型业务结果的保证。
...
- 切换只对后续请求与新连接生效，已有 SSE / WebSocket 保持原上游；不重发失败请求。面板显示等待倒计时、候选排除原因及最近切换，诊断日志记录脱敏切换原因。配置写入失败时沿用现有回滚机制，保留当前路由并提示。
