...
class AutoSwitch {
  ...
  async run(generation) {
    ...
    // Unknown telemetry must neither evict the incumbent nor qualify a new
    // candidate. Confirmed health/call failures and depleted funding still win.
    if (active && active.currentHealth !== 'unavailable' && active.fundingState !== 'unavailable' &&
      (active.currentHealth === 'unknown' || active.fundingState === 'unknown')) {
      this.state = { ...this.state, phase: 'unconfirmed', message: '保留当前中转 · ' +
        (active.fundingState === 'unknown' ? active.fundingReason : active.healthReason) + '；继续检测' };
      return;
    }
    ...
    // Select a healthy replacement in this detection cycle, with no failure grace timer.
    if (!target) {
    ...
    const reason = !active ? '选择池中最优入口'
      : this.failures.has(active.name) ? active.reason
        : active.currentHealth === 'unavailable' ? '当前渠道不可用，立即切换'
          : !active.qualified ? active.reason
